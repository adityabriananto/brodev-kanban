...
app.get('/api/users', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, role, email FROM users ORDER BY name ASC');
...
app.put('/api/tasks/:id', async (req, res) => {
  ...
  try {
    const prevTaskResult = await pool.query('SELECT assigned_hours, title FROM tasks WHERE id = $1', [id]);
    if (prevTaskResult.rows.length === 0) {
      return res.status(404).json({ error: 'Task not found' });
    }
    const prevHours = prevTaskResult.rows[0].assigned_hours;
    const taskTitle = prevTaskResult.rows[0].title;
